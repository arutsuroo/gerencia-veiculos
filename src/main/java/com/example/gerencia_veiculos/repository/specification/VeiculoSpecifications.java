    ...
    public static Specification<Veiculo> filtrar(VeiculoFiltro filtro) {
        return (root, query, cb) -> {
            ...
            if (filtro.getMarca() != null && !filtro.getMarca().isBlank()) {
                predicates.add(cb.equal(cb.lower(root.get("marca")), filtro.getMarca().toLowerCase()));
            }
            ...
            if (filtro.getMinPreco() != null) {
                predicates.add(cb.greaterThanOrEqualTo(root.get("precoUsd"), filtro.getMinPreco()));
            }
            if (filtro.getMaxPreco() != null) {
                predicates.add(cb.lessThanOrEqualTo(root.get("precoUsd"), filtro.getMaxPreco()));
            }
